...
ls -l $destResultPath/${TestCase}_CINT*.ini
...
Node_count=`ls $destResultPath/${TestCase}_CINT*.ini | wc -l`
...
do
  ...
  sh get_spec2000_ncore_pointValue_CINT.sh $TestType $Platform $TestCase $i || { echo "Error!Node [$i]";exit 1; }
done
