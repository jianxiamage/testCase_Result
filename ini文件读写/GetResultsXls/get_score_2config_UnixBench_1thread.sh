...
ls -l $destResultPath/${TestCase}_1thread*.ini
...
Node_count=`ls $destResultPath/${TestCase}_1thread*.ini | wc -l`
...
do
  echo "$TestType $Platform $TestCase (1thread) Node:[$i]"
  sh get_UnixBench_pointValue_1thread.sh $TestType $Platform $TestCase $i || { echo "Error!Node [$i]";exit 1; }
done
