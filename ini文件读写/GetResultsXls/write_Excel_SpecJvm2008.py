...
booksheet = workbook.add_sheet('SpecJvm2008', cell_overwrite_ok=True)
...
def writeResult(TestType,Platform,TestCase,count):

    print count
    IniPath = str(curPointsPath) + '/' + str(TestCase) + '.ini' 
    ExcelPath = ResultPath + str(TestType) + '/' + str(Platform) + '/' + str(TestCase) + '/' + str(PointsPath) + '/' + str(TestCase) +  '_' + str(Platform) + '_' + str(TestType) + '.xls' 
    print IniPath
    print ExcelPath
    ...
    for i in range(1,countNum):
       print '第%d个节点' %(i)
       ResultIniPath = str(ResultPath) + str(TestType) + '/' + str(Platform) + '/' + str(TestCase) + '/' + str(PointsPath) + '/' + str(TestCase) + '_' +  str(i) + '.ini' 
       print ResultIniPath
...
if __name__=='__main__':
  ...
  try:
      ...
      node_count = sys.argv[4]
      
      result_code = writeResult(test_case_type,test_case_platform,test_case,node_count)
      retCode = result_code
      print retCode
