...
ls -l $destResultPath/${TestCase}_1core*.ini
...
Node_count=`ls $destResultPath/${TestCase}_1core*.ini | wc -l`
...
do
  echo "$TestType $Platform $TestCase (1core) Node:[$i]"
  sh get_stream_pointValue_1core.sh $TestType $Platform $TestCase $i || { echo "Error!Node [$i]";exit 1; }
done
