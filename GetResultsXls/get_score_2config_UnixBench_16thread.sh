...
ls -l $destResultPath/${TestCase}_16thread*.ini
...
Node_count=`ls $destResultPath/${TestCase}_16thread*.ini | wc -l`
...
do
  echo "$TestType $Platform $TestCase (16thread) Node:[$i]"
  sh get_UnixBench_pointValue_16thread.sh $TestType $Platform $TestCase $i || { echo "Error!Node [$i]";exit 1; }
done
