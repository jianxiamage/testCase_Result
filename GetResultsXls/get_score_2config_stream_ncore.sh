...
ls -l $destResultPath/${TestCase}_ncore*.ini
...
Node_count=`ls $destResultPath/${TestCase}_ncore*.ini | wc -l`
...
do
  echo "$TestType $Platform $TestCase (ncore) Node:[$i]"
  sh get_stream_pointValue_ncore.sh $TestType $Platform $TestCase $i || { echo "Error!Node [$i]";exit 1; }
done
