...
ls -l $destResultPath/${TestCase}_8thread*.ini
...
Node_count=`ls $destResultPath/${TestCase}_8thread*.ini | wc -l`
...
do
  echo "$TestType $Platform $TestCase (8thread) Node:[$i]"
  sh get_UnixBench_pointValue_8thread.sh $TestType $Platform $TestCase $i || { echo "Error!Node [$i]";exit 1; }
done
