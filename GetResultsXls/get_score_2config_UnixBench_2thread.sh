...
ls -l $destResultPath/${TestCase}_2thread*.ini
...
Node_count=`ls $destResultPath/${TestCase}_2thread*.ini | wc -l`
...
do
  echo "$TestType $Platform $TestCase (2thread) Node:[$i]"
  sh get_UnixBench_pointValue_2thread.sh $TestType $Platform $TestCase $i || { echo "Error!Node [$i]";exit 1; }
done
